...
window.itemManager = itemManager = (function createItemManager() {
  ...
  function createItem(givenCore) {
    ...
    if (typeof givenCore === 'object') {
      ...
      try {
        //but the update function doesn't survive this process, so it is the exception
        var updateFunction;
        if (givenCore.update) {
          updateFunction = givenCore.update;
        }
        core = JSON.parse(JSON.stringify(givenCore));
        //even if updateFunction is still undefined, the program below can deal with it.
        core.update = updateFunction;
      } catch (e) {
        console.log('JSON (json2) did not like createItem argument: ' + e.message);
        core = "Input could not be read by itemManager.createItem().";
      }
    }
    else {
    ...
    if (core && core.localId) {
      ...
      if (typeof core.localId !== 'string') {
        console.log("itemManager.createItem given core.localId that is not string" + core.localId);
      ...
      if (core.localId.length > 64) {
        console.log("itemManager.createItem given core.localId that is > 64: " + core.localId);
      ...
      if (items[core.localId]) {
        console.log("itemManager.createItem tried id but it was already in use: " + core.localId);
      ...
      while (items[newId]) {
        ...
        if (count>9999999999) {//~10 billion
          console.log("Too many identical ids in itemManager.createItem . . . it's about to throw exception!");
        }
        if (count>9999999999999) {//~10 trillion
          throw "Could not find an unused item id in itemManager.createItem";
    ...
    //send this id to the update functions of subscribers
    function publish() {
      var sub;
      for (var i in core.subscriberIds) {
    ...
    function finalizeReciprocationCheck() {
      ...
      for (var i in unverifiedPublishers) {
        ...
        if (!items[id]) {
          console.log(core.localId + " is giving/receiving updates but no sign of its publisher " + id);
        }
      ...
      for (var i in unverifiedSubscribers) {
        ...
        if (!items[id]) {
          console.log(core.localId + " is giving/receiving updates but no sign of its subscriber " + id);
        }
    ...
    function addSubscriberId(subscriberId, priority) {
      ...
      //no immediate update because may cause unwanted behavior
    }
...
//+removePublisherId(publisherId:String):Boolean
    ...
    if (core.update) {
      ...
      //if core.update is there and a function, keep it
    }
    else {
      core.update = function() {};
    }
    ...
    //+update(publisherId:Item):Boolean (this is where others update this; for this updating others, see publish() (called by setDataEtc).)
    function update(publisherId) {
      finalizeReciprocationCheck();
      ...
      core.update.call(items[core.localId], pub);
    }
    ...
    //+setUpdateFunction(update:Function):Boolean
    function setUpdateFunction(update) {
...
window.simplifier = simplifier = (function createSimplifier() {
  ...
  function go() {
    ...
    //act as presenter: pass message and cause model to update presenter
    itemManager.getItem("Easy model").setSomeDataEtc({domElement: "#page"});
  }
  ...
  function createItems() {
    ...
    itemManager.createItem({
      ...
      update: function(publisherId) {
        ...
        var props = itemManager.getItem(publisherId).getDataEtc();
        //get html template from view
        var templateString = itemManager.getItem(props.viewId).getDataEtc().data;
    ...
    itemManager.createItem({
      ...
      ], //TODO make the following default update method? or a common option to be called on?
      update: function(publisherId) {
